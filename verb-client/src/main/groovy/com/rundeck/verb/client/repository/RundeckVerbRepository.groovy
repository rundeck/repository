...
class RundeckVerbRepository implements VerbArtifactRepository {
    ...
    ResponseBatch saveNewArtifact(final VerbArtifact verbArtifact) {
        ...
        ArtifactUtils.saveArtifactToOutputStream(verbArtifact,baos)
        RequestBody body = RequestBody.create(MediaType.parse("application/octet-stream"), baos.toByteArray())
        Response response = null
        try {
            OkHttpClient client = new OkHttpClient();
            Request rq = new Request.Builder().method("POST", body).url(rundeckVerbEndpoint+"/upload/artifact/${verbArtifact.artifactMetaFileName}".toString()).build()
            response = client.newCall(rq).execute()
    ...
    ResponseBatch uploadArtifactBinary(final RundeckVerbArtifact rundeckVerbArtifact, final File binaryFile) {
        ...
        Response response = null
        try {
            Request rq = new Request.Builder().method("POST", body).url(rundeckVerbEndpoint+"/upload/binary/${rundeckVerbArtifact.artifactBinaryFileName}".toString()).build()
            response = client.newCall(rq).execute()
            rbatch.addMessage(ResponseMessage.success())
        } catch(Exception ex) {
            ex.printStackTrace()
